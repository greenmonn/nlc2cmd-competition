...
class tokenizer(object):
    ...
    def _readtokenword(self, c):
        ...
        def handleshellexp():
            ...
            if peek_char == '(' or (c == '$' and peek_char in '{['):
                ...
                if peek_char == '{':
                    ...
                    ttok = self._parse_comsub(
                        cd, '(', ')', parsingcommand=True)
                    self._pop_delimiter()
                else:
        ...
        tokenword = self._createtoken(
            tokentype.WORD, tokenword, butils.typedset(wordflags))
        if d['dollar_present']:
    ...
    def _parse_comsub(self, doublequotes, open, close, parsingcommand=False,
        ...
        checkcase = bool(parsingcommand and (
            doublequotes is None or doublequotes not in "'\"") and not dquote)
        ...
        while count:
            ...
            if c is None:
                raise MatchedPairError(
                    startlineno, 'unexpected EOF while looking for matching %r' % close, self)
            ...
            if not reservedwordok and checkcase and not insidecomment and (_shellmeta(c) or c == '\n'):
                ...
                if c == peekc and c in '&|;':
                    ...
                    raise MatchedPairError(
                        startlineno, 'unexpected EOF while looking for matching %r' % close, self)  # pragma: no coverage
                else:
                    ret = ret[:-1]
                    self._ungetc(peekc)
    ...
    def _parse_matched_pair(self, doublequotes, open, close, parsingcommand=False, allowesc=False, dquote=False, firstclose=False, dolbrace=False, arraysub=False):
        ...
        while count:
            ...
            if open != close:
                    ...
                    try:
                        if sawdollar and "'":
                            nestret = self._parse_matched_pair(
                                c, c, c, parsingcommand=parsingcommand, allowesc=True, dquote=dquote, firstclose=firstclose, dolbrace=dolbrace)
                        else:
                            nestret = self._parse_matched_pair(
                                c, c, c, parsingcommand=parsingcommand, allowesc=allowesc, dquote=dquote, firstclose=firstclose, dolbrace=dolbrace)
                    finally:
                        self._pop_delimiter()
                ...
                ret += self._parse_matched_pair(None, '`', '`', parsingcommand=parsingcommand,
                                                allowesc=allowesc, dquote=dquote, firstclose=firstclose, dolbrace=dolbrace)
