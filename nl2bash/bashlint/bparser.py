...
def p_shell_command(p):
    ...
    if len(p) == 2:
        ...
        p[0] = bast.node(kind='compound',
                         redirects=[],
                         list=[bast.node(kind=kind, parts=parts,
...
def p_for_command(p):
    ...
    p[0] = bast.node(kind='compound',
                     redirects=[],
                     list=[bast.node(kind='for', parts=parts,
                                     pos=_partsspan(parts))],
                     pos=_partsspan(parts))
...
def p_if_command(p):
    ...
    p[0] = bast.node(kind='compound',
                     redirects=[],
                     list=[bast.node(kind='if', parts=parts,
                                     pos=_partsspan(parts))],
                     pos=_partsspan(parts))
...
def p_elif_clause(p):
    ...
    for i in range(1, len(p)):
        if isinstance(p[i], bast.node):
            ...
            parts.append(bast.node(kind='reservedword',
                                   word=p[i], pos=p.lexspan(i)))
    p[0] = parts
...
class _parser(object):
    ...
    def __init__(self, s, strictmode=True, expansionlimit=None, tokenizerargs=None):
        ...
        if tokenizerargs is None:
            tokenizerargs = {}
        self.parserstate = tokenizerargs.pop(
