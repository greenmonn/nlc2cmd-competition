...
class UnaryLogicOpNode(Node):
    num_child = 1
    children_types = [
        set(['flag', 'bracket', 'unarylogicop', 'binarylogicop'])]
    LEFT = 0
    RIGHT = 1

    def __init__(self, value='', parent=None, lsb=None):
        super(UnaryLogicOpNode, self).__init__(
            parent, lsb, 'unarylogicop', value)
        if value in bash.right_associate_unary_logic_operators:
            ...
            raise ValueError(
                "Unrecognized unary logic operator: {}".format(value))
...
class BinaryLogicOpNode(Node):
    ...
    def __init__(self, value='', parent=None, lsb=None):
        super(BinaryLogicOpNode, self).__init__(
            parent, lsb, 'binarylogicop', value)
...
class ProcessSubstitutionNode(Node):
    ...
    def __init__(self, value, parent=None, lsb=None):
        ...
        if value in ["<", ">"]:
            ...
            raise ValueError(
                "Value of a processsubstitution has to be '<' or '>'.")
