...
def gen_manual_evaluation_csv_single_model(dataset, FLAGS):
    ...
    with open(output_path, 'w') as o_f:
        ...
        for example_id in range(len(grouped_dataset)):
            ...
            command_gt_asts = [data_tools.bash_parser(
                cmd) for cmd in command_gts]
            ...
            template_gt_asts = [data_tools.bash_parser(
                temp) for temp in template_gts]
            ...
            for i in xrange(3):
                ...
                pred_temp = data_tools.ast2template(
                    pred_tree, loose_constraints=True)
    ...
    for example_id in sample_ids:
        ...
        for model_name in ['char-seq2seq',
            ...
            if model_name == 'char-seq2seq':
                print(
                    '\\multirow{{7}}{{*}}{{\\specialcell{{{}}}}} '.format(sc_txt) + output_strs[model_name])
            else:
