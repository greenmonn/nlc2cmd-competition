...
def sentence_bleu_score(gt_asts, pred_ast):
    gt_tokens = [data_tools.bash_tokenizer(
        ast, ignore_flag_order=True) for ast in gt_asts]
    pred_tokens = data_tools.bash_tokenizer(
        pred_ast, loose_constraints=True, ignore_flag_order=True)
...
def corpus_bleu_score(gt_asts_list, pred_ast_list):
    gt_tokens_list = [[data_tools.bash_tokenizer(
        ast, ignore_flag_order=True) for ast in gt_asts] for gt_asts in gt_asts_list]
    pred_tokens_list = [data_tools.bash_tokenizer(
        pred_ast, loose_constraints=True, ignore_flag_order=True) for pred_ast in pred_ast_list]
