...
def define_model(FLAGS, session, model_constructor, buckets, forward_only):
    ...
    if forward_only or FLAGS.gen_slot_filling_training_data or \
        ...
        if not os.path.isdir(os.path.dirname(ckpt.model_checkpoint_path)):
            ckpt.model_checkpoint_path = os.path.abspath(
                ckpt.model_checkpoint_path)
            print(ckpt.model_checkpoint_path)
            ckpt.model_checkpoint_path = ckpt.model_checkpoint_path.replace(
                os.path.abspath(os.path.join(ckpt.model_checkpoint_path, '../../..')), '/nlc2cmd/src/submission_code/nlc2cmd/nl2bash/model')
...
def _reverse_seq(input_seq, lengths):
    ...
    s_reversed = tf.reverse_sequence(
        input=s_joined, seq_lengths=lengths, seq_axis=0, batch_axis=1)
