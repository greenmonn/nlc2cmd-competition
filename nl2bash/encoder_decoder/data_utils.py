    ...
    if load_features and use_buckets:
        ...
        if split == 'train':
            ...
            sorted_dataset = sorted(
                dataset, key=lambda x: len(x.sc_ids), reverse=True)
            max_sc_length = len(
                sorted_dataset[int(len(sorted_dataset) * length_cutoff)].sc_ids)
            # B. Determine maximum target length
            sorted_dataset = sorted(
                dataset, key=lambda x: len(x.tg_ids), reverse=True)
            max_tg_length = len(
                sorted_dataset[int(len(sorted_dataset) * length_cutoff)].tg_ids)
            ...
            buckets = []
            sorted_dataset = sorted(
                dataset, key=lambda x: len(x.sc_ids), reverse=False)
            max_tg_len_so_far = 0
            for i, dp in enumerate(sorted_dataset):
                ...
                if i > 0 and i % bucket_capacity == 0:
                    buckets.append(
                        (len(dp.sc_ids)+1, min(max_tg_len_so_far, max_tg_length)+1))
...
def load_vocabulary(FLAGS):
    ...
    source_vocab_path = os.path.join(
        data_dir, '{}.{}'.format(source, vocab_ext))
    target_vocab_path = os.path.join(
        data_dir, '{}.{}'.format(target, vocab_ext))
...
def parallel_data_to_partial_tokens(nl_list, cm_list):
    nl_data = [nl_to_partial_tokens(
        nl, tokenizer.basic_tokenizer) for nl in nl_list]
    cm_data = [cm_to_partial_tokens(
        cm, data_tools.bash_tokenizer) for cm in cm_list]
    return nl_data, cm_data
...
def nl_to_characters(nl, use_preprocessing=False):
    ...
    if use_preprocessing:
        nl_tokens = nl_to_tokens(
            nl, tokenizer.basic_tokenizer, lemmatization=False)
...
def compute_alignments(data_dir, nl_list, cm_list, split, channel):
    ...
    output_path = os.path.join(
        data_dir, '{}.{}.align.readable'.format(split, channel))
    with open(output_path, 'w') as o_f:
        for nl_tokens, cm_tokens in zip(nl_list, cm_list):
            alignments.append(compute_pair_alignment(
                nl_tokens, cm_tokens, o_f))
    return alignments
...
def group_parallel_data(dataset, attribute='source', use_temp=False,
    ...
    if dataset.data_points and isinstance(dataset.data_points, list):
        if isinstance(dataset.data_points[0], list):
            data_points = functools.reduce(
                lambda x, y: x + y, dataset.data_points)
        else:
