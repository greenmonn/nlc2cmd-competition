...
def define_input_flags():
    ...
    tf.compat.v1.flags.DEFINE_integer('steps_per_epoch', 4000,
                                      'How many training steps to do per checkpoint.')
    tf.compat.v1.flags.DEFINE_integer('num_epochs', 100,
    ...
    tf.compat.v1.flags.DEFINE_string(
        'gpu', '0', 'GPU device where the computation is going to be placed.')
    tf.compat.v1.flags.DEFINE_boolean(
        'log_device_placement', False, 'Set to True for logging device placement.')

    # data hyperparameters
    tf.compat.v1.flags.DEFINE_string(
        'dataset', 'bash', 'select dataset to use.')
    tf.compat.v1.flags.DEFINE_string(
        'channel', 'token', 'translation feature channel.')
    tf.compat.v1.flags.DEFINE_string('data_dir', os.path.join(
        os.path.dirname(__file__), 'data'), 'Data directory')
    tf.compat.v1.flags.DEFINE_string('model_dir', os.path.join(
        os.path.dirname(__file__), 'model'), 'Model directory')
    tf.compat.v1.flags.DEFINE_string(
        'prediction_file', None, 'path to where the decoding output is stored')
    ...
    tf.compat.v1.flags.DEFINE_integer(
        'max_sc_length', 100, 'Maximum source sequence length.')
    tf.compat.v1.flags.DEFINE_integer(
        'max_tg_length', 100, 'Maximum target sequence length.')
    tf.compat.v1.flags.DEFINE_integer(
        'sc_vocab_size', 1000, 'Source vocabulary size.')
    tf.compat.v1.flags.DEFINE_integer(
        'tg_vocab_size', 1000, 'Target vocabulary size.')
    tf.compat.v1.flags.DEFINE_integer(
        'max_sc_token_size', 100, 'Maximum source token size.')
    tf.compat.v1.flags.DEFINE_integer(
        'max_tg_token_size', 100, 'Maximum target token size.')
    tf.compat.v1.flags.DEFINE_integer('min_vocab_frequency', 4,
                                      'Minimum frequency of token in the dataset that are not considered UNK.')
    tf.compat.v1.flags.DEFINE_integer(
        'num_buckets', 3, 'Number of buckets to use.')
    ...
    tf.compat.v1.flags.DEFINE_float(
        'adam_epsilon', 1e-08, 'Epsilon parameter in the Adam Optimizer.')
    ...
    tf.compat.v1.flags.DEFINE_integer('num_samples', 256,
                                      'Number of samples for sampled softmax.')
    tf.compat.v1.flags.DEFINE_integer(
        'seed', -1, 'Random seed for graph initialization.')

    tf.compat.v1.flags.DEFINE_boolean('variational_recurrent_dropout', True, 'Set to use variational ' +
                                      'recurrent dropout on the RNN cells.')
    tf.compat.v1.flags.DEFINE_float('universal_keep', 0.6, 'Use the rate for all ' +
                                    'dropout layers if set to a number between 0 and 1.')
    tf.compat.v1.flags.DEFINE_float(
        'sc_input_keep', 1.0, 'Proportion of source input to keep if dropout is used.')
    tf.compat.v1.flags.DEFINE_float(
        'sc_output_keep', 1.0, 'Proportion of source output to keep if dropout is used.')
    tf.compat.v1.flags.DEFINE_float(
        'tg_input_keep', 1.0, 'Proportion of target input to keep if dropout is used.')
    tf.compat.v1.flags.DEFINE_float(
        'tg_output_keep', 1.0, 'Proportion of target output to keep if dropout is used.')

    tf.compat.v1.flags.DEFINE_boolean('recurrent_batch_normalization', False,
                                      'Set to use recurrent batch normalization.')
    tf.compat.v1.flags.DEFINE_float(
        'gamma_c', .1, 'Scale of cell state normalization.')
    tf.compat.v1.flags.DEFINE_float(
        'beta_c', .1, 'Offset of cell state normalization.')
    tf.compat.v1.flags.DEFINE_float(
        'gamma_h', .1, 'Scale of hidden state normalization.')
    tf.compat.v1.flags.DEFINE_float(
        'beta_h', .1, 'Offset of hidden state normalization.')
    tf.compat.v1.flags.DEFINE_float(
        'gamma_x', .1, 'Scale of input state normalization.')
    tf.compat.v1.flags.DEFINE_float(
        'beta_x', .1, 'Offset of input state normalization.')

    tf.compat.v1.flags.DEFINE_string(
        'training_algorithm', 'standard', 'training algorithm to use.')
    tf.compat.v1.flags.DEFINE_string(
        'pretrained_model_subdir', '', 'signature of pretrained model.')

    tf.compat.v1.flags.DEFINE_string(
        'encoder_topology', 'birnn', 'structure of the encoder.')
    tf.compat.v1.flags.DEFINE_string(
        'decoder_topology', 'rnn', 'structure of the decoder.')

    tf.compat.v1.flags.DEFINE_boolean(
        'tg_token_use_attention', True, 'If set, use attention for token decoder.')
    tf.compat.v1.flags.DEFINE_boolean(
        'tg_char_use_attention', False, 'If set, use attention for char decoder.')
    tf.compat.v1.flags.DEFINE_string('tg_token_attn_fun', 'non-linear',
    ...
    tf.compat.v1.flags.DEFINE_float(
        'beta', 0, 'Attention regularization weight.')
    tf.compat.v1.flags.DEFINE_float('attention_input_keep', 1.0,
                                    'Proportion of attention input state to keep if dropout is used.')
    tf.compat.v1.flags.DEFINE_float('attention_output_keep', 1.0,
    ...
    tf.compat.v1.flags.DEFINE_integer(
        'beam_size', 100, 'Size of beam for beam search.')
    tf.compat.v1.flags.DEFINE_integer(
        'beam_order', -1, 'Order for beam search.')
    tf.compat.v1.flags.DEFINE_float(
        'alpha', 1.0, 'Beam search length normalization parameter.')
    tf.compat.v1.flags.DEFINE_integer(
    ...
    tf.compat.v1.flags.DEFINE_integer('num_nn_slot_filling', 10, 'Number of nearest neighbors to use in '
    ...
    tf.compat.v1.flags.DEFINE_integer(
        'sc_token_embedding_size', 1000, 'source word embedding size.')
    tf.compat.v1.flags.DEFINE_integer(
        'sc_token_dim', 200, 'Basic token embedding dimensions.')
    ...
    tf.compat.v1.flags.DEFINE_integer(
        'sc_char_dim', 300, 'Dimension of each character embeddings.')
    tf.compat.v1.flags.DEFINE_string(
        'sc_char_composition', 'rnn', 'Specify the character to token composition function.')
    tf.compat.v1.flags.DEFINE_string(
        'sc_char_rnn_cell', 'gru', 'Type of RNN cell to use for the character model.')
    ...
    tf.compat.v1.flags.DEFINE_integer(
        'tg_token_embedding_size', 1000, 'target word embedding size.')
    ...
    tf.compat.v1.flags.DEFINE_string(
        'tg_char_rnn_cell', 'gru', 'Type of RNN cell to use for the character model.')
    ...
    tf.compat.v1.flags.DEFINE_float(
        'gamma', 5, 'Define the weight of the character channel loss.')
    ...
    # debugging options
    tf.compat.v1.flags.DEFINE_boolean(
        'force_reading_input', False, 'If set, read ground truth decoder inputs for decoding.')
    ...
    tf.compat.v1.flags.DEFINE_string('annotation_filepath', '',
                                     'Additional arguments added due to the clai submission framework')
