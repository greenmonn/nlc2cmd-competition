...
class BeamDecoderCellWrapper(tf.compat.v1.nn.rnn_cell.RNNCell):
    def __init__(self, cell, output_project, num_layers,
        ...
        self.seq_len = tf.constant(
            1e-12, shape=[self.full_size], dtype=tf.float32)

    def __call__(self, cell_inputs, state, scope=None):
        (
            # [batch_size*self.beam_size, :], right-aligned!!!
            past_beam_symbols,
        ...
        logprobs = tf.multiply(
            logprobs, (1 - tf.multiply(stop_mask, done_mask)))
        ...
        logprobs_batched = tf.compat.v1.div(
            logprobs_unormalized, tf.pow(seq_len, self.alpha))
        ...
        parent_refs = tf.reshape(indices // num_classes, [-1])
        ...
        def concat_and_gather_tuple_states(pc_states, c_state):
            rc_states = (
                tf.concat(axis=1, values=[
                          pc_states[0], tf.expand_dims(c_state[0], 1)]),
                tf.concat(axis=1, values=[
                          pc_states[1], tf.expand_dims(c_state[1], 1)])
            )
            c_states = (
                nest_map(lambda element: tf.gather(
                    element, parent_refs), rc_states[0]),
                nest_map(lambda element: tf.gather(
                    element, parent_refs), rc_states[1])
            )
            return c_states
        ...
        if nest.is_sequence(cell_state):
            ...
            ranked_cell_states = tf.gather(
                tf.concat(axis=1, values=[
                          past_cell_states, tf.expand_dims(cell_state, 1)]),
                parent_refs)
    ...
    def _create_state(self, batch_size, dtype, cell_state=None):
        if cell_state is None:
            cell_state = self.cell.zero_state(
                batch_size*self.beam_size, dtype=dtype)
        else:
            cell_state = BeamDecoder._tile_along_beam(
                self.beam_size, cell_state)
...
def sparse_boolean_mask(tensor, mask):
    ...
    """
    mask_lens = tf.reduce_sum(input_tensor=tf.cast(
        mask, tf.int32), axis=-1, keepdims=True)
