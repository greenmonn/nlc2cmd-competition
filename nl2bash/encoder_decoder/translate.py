...
def train(train_set, test_set, verbose=False):
    with tf.compat.v1.Session(config=tf.compat.v1.ConfigProto(allow_soft_placement=True,
        ...
        model = define_model(sess, forward_only=False,
        ...
        for t in xrange(FLAGS.num_epochs):
            ...
            for _ in tqdm(xrange(FLAGS.steps_per_epoch)):
                ...
                formatted_example = model.get_batch(
                    train_set.data_points, bucket_id)
            ...
            if t % FLAGS.epochs_per_checkpoint == 0:
                ...
                checkpoint_path = os.path.join(
                    FLAGS.model_dir, "translate.ckpt")
                ...
                repeated_samples = list(
                    range(len(train_set.buckets))) * sample_size
                for bucket_id in repeated_samples:
                    if len(test_set.data_points[bucket_id]) == 0:
                        ...
                        continue
                    formatted_example = model.get_batch(
                        test_set.data_points, bucket_id)
                    model_outputs = model.step(
                ...
                dev_perplexity = math.exp(
                    dev_loss) if dev_loss < 1000 else float('inf')
...
def predict(invocations):
    # generate predictions from given data
    ...
    if FLAGS.process_data:
        ...
        if FLAGS.eval:
            ...
            if FLAGS.test:
                ...
                eval_tools.gen_manual_evaluation_table(
                    dataset, FLAGS, num_examples=100)
            ...
            error_analysis.tabulate_example_predictions(
                dataset, FLAGS, num_examples=100)
        else:
            train_set, dev_set, test_set = data_utils.load_data(
