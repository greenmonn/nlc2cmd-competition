...
class BasicTreeDecoder(decoder.Decoder):
    ...
    def __init__(self, hyperparams, scope, dim, embedding_dim, use_attention,
                 attention_function, input_keep, output_keep, decoding_algorithm):
        super(BasicTreeDecoder, self).__init__(
            hyperparams, scope, dim, embedding_dim, use_attention, attention_function,
            input_keep, output_keep, decoding_algorithm)

        self.H_NO_EXPAND = tf.constant(
            data_utils.H_NO_EXPAND_ID, shape=[self.batch_size])
        self.V_NO_EXPAND = tf.constant(
            data_utils.V_NO_EXPAND_ID, shape=[self.batch_size])

    def define_graph(self, encoder_state, decoder_inputs, 
                     embeddings=None, encoder_attn_masks=None,
                     attention_states=None, num_heads=1,
                     initial_state_attention=False, feed_previous=False,
                     reuse_variables=False, encoder_copy_inputs=None):
        """
        ...
        with tf.compat.v1.variable_scope("basic_tree_decoder") as scope:
            ...
            if self.rnn_cell == "gru":
                ...
                init_state = tf.concat(
                    axis=1, values=[encoder_state[0], encoder_state[1]])
            ...
            for i in xrange(len(decoder_inputs)):
                ...
                if self.use_attention:
                    ...
                    v_output, v_state = vertical_cell(
                        input_embeddings, state, vertical_scope)
                    h_output, h_state = horizontal_cell(
                        input_embeddings, state, horizontal_scope)
                ...
                batch_output = switch_mask_fun(switch_mask, [h_output, v_output])
                if self.rnn_cell == "gru":
                    batch_state = switch_mask_fun(switch_mask, [h_state, v_state])
                elif self.rnn_cell == "lstm":
                    batch_cell = switch_mask_fun(
                        switch_mask, [h_state[0], v_state[0]])
                    batch_hs = switch_mask_fun(
                        switch_mask, [h_state[1], v_state[1]])
                    batch_state = tf.concat(
                        axis=1, values=[batch_cell, batch_hs])

                if self.use_attention:
                    batch_attns = switch_mask_fun(switch_mask, [h_attns, v_attns])
                    batch_state = tf.concat(
                        axis=1, values=[batch_state, batch_attns])
                ...
                if i < len(decoder_inputs) - 1:
                    ...
                    if feed_previous:
                        ...
                        batch_output_symbol = tf.argmax(
                            input=batch_projected_output, axis=1)
                        batch_output_symbol = tf.cast(
                            batch_output_symbol, dtype=tf.int32)
                    else:
                        batch_output_symbol = decoder_inputs[i+1]
                    search_left_to_right_next = self.is_no_expand(
                        batch_output_symbol)
                    ...
                    if DEBUG:
                        print("back_pointer.get_shape(): {}".format(
                            back_pointer.get_shape()))
                    ...
                    if DEBUG:
                        print("next_input.get_shape(): {}".format(
                            next_input.get_shape()))
                    ...
                    next_state = map_fn(
                        ...
                        self.batch_size)
                    if DEBUG:
                        print("next_state.get_shape(): {}".format(
                            next_state.get_shape()))
        ...
        if self.rnn_cell == "gru":
            ...
            final_batch_state = tf.compat.v1.nn.rnn_cell.LSTMStateTuple(
                batch_cell, batch_hs)

        if self.use_attention:
            temp = [tf.expand_dims(batch_attn_alignment, 1)
    ...
    def back_pointer(self, x):
        ...
        return tf.cond(pred=h_search_next,
                       true_fn=lambda: tf.cond(
                           pred=h_search[0], true_fn=lambda: grandparent, false_fn=lambda: parent),
    ...
    def next_input(self, x):
        ...
        return tf.cond(pred=h_search_next,
                       true_fn=lambda: tf.cond(
                           pred=h_search[0], true_fn=lambda: parent, false_fn=lambda: current),
    ...
    def parent_input(self):
        inds = tf.nn.embedding_lookup(params=self.E, ids=tf.split(
            axis=0, num_or_size_splits=self.batch_size, value=self.parent()))
    ...
    def parent_state(self):
        ...
        inds = tf.tile(inds, tf.stack(
            [tf.constant(1), tf.constant(1), tf.shape(input=self.state)[2]]))
    ...
    def push(self, batch_states):
        ...
        batch_back_pointers = tf.expand_dims(batch_back_pointers, 1)
        self.back_pointers = tf.concat(
            axis=1, values=[self.back_pointers, batch_back_pointers])
    ...
    def peek(self):
        ...
        if self.rnn_cell == "gru":
            ...
            batch_states = tf.compat.v1.nn.rnn_cell.LSTMStateTuple(
                batch_stack_cells, batch_stack_hiddens)
        else:
...
def map_fn(fn, elems, batch_size):
    ...
    results = []
    elem_lists = [
        tf.split(axis=0, num_or_size_splits=batch_size, value=elem) for elem in elems]
