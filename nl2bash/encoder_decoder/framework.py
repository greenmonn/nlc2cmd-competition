...
class EncoderDecoderModel(graph_utils.NNModel):
    ...
    def define_graph(self):
        ...
        # decoder inputs (always start with "_GO").
        self.decoder_inputs = []
        self.targets = []               # decoder targets
        # weights at each position of the target sequence.
        self.target_weights = []
        ...
        if self.buckets:
            ...
            for bucket_id, bucket in enumerate(self.buckets):
                with tf.compat.v1.variable_scope(tf.compat.v1.get_variable_scope(),
                    ...
                    self.output_symbols.append(
                        encode_decode_outputs['output_symbols'])
                    self.sequence_logits.append(
                        encode_decode_outputs['sequence_logits'])
                    self.losses.append(encode_decode_outputs['losses'])
                    self.attn_alignments.append(
                        encode_decode_outputs['attn_alignments'])
            ...
            if self.buckets:
                ...
                for bucket_id, _ in enumerate(self.buckets):
                    gradients = tf.gradients(
                        ys=self.losses[bucket_id], xs=params)
    ...
    def feed_input(self, E):
        ...
        """
        encoder_size, decoder_size = len(
            E.encoder_inputs), len(E.decoder_inputs)
        input_feed = {}
    ...
    def step(self, session, formatted_example, bucket_id=-1, forward_only=False):
        ...
        if not forward_only:
            if bucket_id == -1:
                ...
                output_feed = {
                    # Update Op that does SGD.
                    'updates': self.updates[bucket_id],
                    # Gradient norm.
                    'gradient_norms': self.gradient_norms[bucket_id],
            ...
            if bucket_id == -1:
                ...
                output_feed = {
                    # Loss for this batch.
                    'output_symbols': self.output_symbols[bucket_id],
                    # Batch output sequence
                    'sequence_logits': self.sequence_logits[bucket_id],
